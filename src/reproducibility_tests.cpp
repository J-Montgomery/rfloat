...
TEST(IntegerTests, RandomInputsFloor) {
    ...
    for (const auto &param : test_data) {
        auto results = floor(param.inputs[0]);
        EXPECT_EQ(results, param.expected_outputs[0]);
...
TEST(IntegerTests, RandomInputsCeil) {
    ...
    for (const auto &param : test_data) {
        auto results = ceil(param.inputs[0]);
        EXPECT_EQ(results, param.expected_outputs[0]);
...
TEST(IntegerTests, RandomInputsRound) {
    ...
    for (const auto &param : test_data) {
        auto results = round(param.inputs[0]);
        EXPECT_EQ(results, param.expected_outputs[0]);
...
TEST(IntegerTests, RandomInputsTrunc) {
    ...
    for (const auto &param : test_data) {
        auto results = trunc(param.inputs[0]);
        EXPECT_EQ(results, param.expected_outputs[0]);
...
TEST(IntegerTests, RandomInputsNearbyInt) {
    ...
    for (const auto &param : test_data) {
        auto results = nearbyint(param.inputs[0]);
        EXPECT_EQ(results, param.expected_outputs[0]);
...
TEST(IntegerTests, RandomInputsRint) {
    ...
    for (const auto &param : test_data) {
        auto results = rint(param.inputs[0]);
        EXPECT_EQ(results, param.expected_outputs[0]);
...
TEST(BasicTest, RandomInputsAbs) {
    ...
    for (const auto &param : test_data) {
        auto results = abs(param.inputs[0]);
        EXPECT_EQ(results, param.expected_outputs[0]);
...
TEST(BasicTest, RandomInputsFMA) {
    ...
    for (const auto &param : test_data) {
        auto results = fma(param.inputs[0], param.inputs[1], param.inputs[2]);
        EXPECT_EQ(results, param.expected_outputs[0]);
...
TEST(BasicTest, RandomInputsSqrt) {
    ...
    for (const auto &param : test_data) {
        auto results = sqrt(param.inputs[0]);
        EXPECT_EQ(results, param.expected_outputs[0]);
...
TEST(BasicTest, RandomInputsNextAfter) {
    ...
    for (const auto &param : test_data) {
        auto results = nextafter(param.inputs[0], param.inputs[1]);
        EXPECT_EQ(results, param.expected_outputs[0]);
...
TEST(BasicTest, RandomInputsNextToward) {
    ...
    for (const auto &param : test_data) {
        auto results = nexttoward(param.inputs[0], param.inputs[1]);
        EXPECT_EQ(results, param.expected_outputs[0]);
...
TEST(BasicTest, RandomInputsFmod) {
    ...
    for (const auto &param : test_data) {
        auto results = fmod(param.inputs[0], param.inputs[1]);
        EXPECT_EQ(results, param.expected_outputs[0]);
...
TEST(BasicTest, RandomInputsRemainder) {
    ...
    for (const auto &param : test_data) {
        auto results = remainder(param.inputs[0], param.inputs[1]);
        EXPECT_EQ(results, param.expected_outputs[0]);
...
TEST(BasicTest, RandomInputsFdim) {
    ...
    for (const auto &param : test_data) {
        auto results = fdim(param.inputs[0], param.inputs[1]);
        EXPECT_EQ(results, param.expected_outputs[0]);
...
TEST(BasicTest, RandomInputsFmax) {
    ...
    for (const auto &param : test_data) {
        auto results = fmax(param.inputs[0], param.inputs[1]);
        EXPECT_EQ(results, param.expected_outputs[0]);
...
TEST(BasicTest, RandomInputsFmin) {
    ...
    for (const auto &param : test_data) {
        auto results = fmin(param.inputs[0], param.inputs[1]);
        EXPECT_EQ(results, param.expected_outputs[0]);
...
TEST(ComparisonTestsTest, RandomIsGreater) {
    ...
    for (const auto &param : test_data) {
        auto results = isgreater(param.inputs[0], param.inputs[1]);
        EXPECT_EQ(results, param.expected_outputs[0]);
...
TEST(ComparisonTestsTest, RandomIsGreaterEqual) {
    ...
    for (const auto &param : test_data) {
        auto results = isgreaterequal(param.inputs[0], param.inputs[1]);
        EXPECT_EQ(results, param.expected_outputs[0]);
        EXPECT_EQ((results == 1.0),
...
TEST(ComparisonTestsTest, RandomIsLess) {
    ...
    for (const auto &param : test_data) {
        auto results = isless(param.inputs[0], param.inputs[1]);
        EXPECT_EQ(results, param.expected_outputs[0]);
        EXPECT_EQ((results == 1.0),
...
TEST(ComparisonTestsTest, RandomIsLessEqual) {
    ...
    for (const auto &param : test_data) {
        auto results = islessequal(param.inputs[0], param.inputs[1]);
        EXPECT_EQ(results, param.expected_outputs[0]);
        EXPECT_EQ((results == 1.0),
...
TEST(ComparisonTestsTest, RandomIsLessGreater) {
    ...
    for (const auto &param : test_data) {
        auto results = islessgreater(param.inputs[0], param.inputs[1]);
        EXPECT_EQ(results, param.expected_outputs[0]);
        EXPECT_EQ((results == 1.0),
...
TEST(TrigTests, RandomSin) {
    ...
    for (const auto &param : test_data) {
        auto result = sin(param.inputs[0]);
        EXPECT_EQ(result, param.expected_outputs[0]);
...
TEST(TrigTests, RandomCos) {
    ...
    for (const auto &param : test_data) {
        auto result = cos(param.inputs[0]);
        EXPECT_EQ(result, param.expected_outputs[0]);
...
TEST(TrigTests, RandomTan) {
    ...
    for (const auto &param : test_data) {
        auto result = tan(param.inputs[0]);
        EXPECT_EQ(result, param.expected_outputs[0]);
...
TEST(TrigTests, RandomAsin) {
    ...
    for (const auto &param : test_data) {
        auto result = asin(param.inputs[0]);
        EXPECT_EQ(result, param.expected_outputs[0]);
...
TEST(TrigTests, RandomAcos) {
    ...
    for (const auto &param : test_data) {
        auto result = acos(param.inputs[0]);
        EXPECT_EQ(result, param.expected_outputs[0]);
...
TEST(TrigTests, RandomAtan) {
    ...
    for (const auto &param : test_data) {
        auto result = atan(param.inputs[0]);
        EXPECT_EQ(result, param.expected_outputs[0]);
...
TEST(TrigTests, RandomAtan2) {
    ...
    for (const auto &param : test_data) {
        auto result = atan2(param.inputs[0], param.inputs[1]);
        EXPECT_EQ(result, param.expected_outputs[0]);
...
TEST(TrigTests, RandomHypot) {
    ...
    for (const auto &param : test_data) {
        auto result = hypot(param.inputs[0], param.inputs[1]);
        EXPECT_EQ(result, param.expected_outputs[0]);
...
TEST(LogExpTests, RandomLog) {
    ...
    for (const auto &param : test_data) {
        auto result = log(param.inputs[0]);
        EXPECT_EQ(result, param.expected_outputs[0]);
...
TEST(LogExpTests, RandomExp) {
    ...
    for (const auto &param : test_data) {
        auto result = exp(param.inputs[0]);
        EXPECT_EQ(result, param.expected_outputs[0]);
...
TEST(LogExpTests, RandomLog2) {
    ...
    for (const auto &param : test_data) {
        auto result = log2(param.inputs[0]);
        EXPECT_EQ(result, param.expected_outputs[0]);
...
TEST(LogExpTests, RandomLog10) {
    ...
    for (const auto &param : test_data) {
        auto result = log10(param.inputs[0]);
        EXPECT_EQ(result, param.expected_outputs[0]);
...
TEST(LogExpTests, RandomExp2) {
    ...
    for (const auto &param : test_data) {
        auto result = exp2(param.inputs[0]);
        EXPECT_EQ(result, param.expected_outputs[0]);
...
TEST(LogExpTests, RandomLog1p) {
    ...
    for (const auto &param : test_data) {
        auto result = log1p(param.inputs[0]);
        EXPECT_EQ(result, param.expected_outputs[0]);
...
TEST(LogExpTests, RandomExpm1) {
    ...
    for (const auto &param : test_data) {
        auto result = expm1(param.inputs[0]);
        EXPECT_EQ(result, param.expected_outputs[0]);
...
TEST(PowTests, RandomPow) {
    ...
    for (const auto &param : test_data) {
        auto result = pow(param.inputs[0], param.inputs[1]);
        EXPECT_EQ(result, param.expected_outputs[0]);
...
TEST(PowTests, RandomCbrt) {
    ...
    for (const auto &param : test_data) {
        auto result = cbrt(param.inputs[0]);
        EXPECT_EQ(result, param.expected_outputs[0]);
...
TEST(HyperbolicTests, RandomSinh) {
    ...
    for (const auto &param : test_data) {
        auto result = sinh(param.inputs[0]);
        EXPECT_EQ(result, param.expected_outputs[0]);
...
TEST(HyperbolicTests, RandomCosh) {
    ...
    for (const auto &param : test_data) {
        auto result = cosh(param.inputs[0]);
        EXPECT_EQ(result, param.expected_outputs[0]);
...
TEST(HyperbolicTests, RandomTanh) {
    ...
    for (const auto &param : test_data) {
        auto result = tanh(param.inputs[0]);
        EXPECT_EQ(result, param.expected_outputs[0]);
...
TEST(HyperbolicTests, RandomAsinh) {
    ...
    for (const auto &param : test_data) {
        auto result = asinh(param.inputs[0]);
        EXPECT_EQ(result, param.expected_outputs[0]);
...
TEST(HyperbolicTests, RandomAcosh) {
    ...
    for (const auto &param : test_data) {
        auto result = acosh(param.inputs[0]);
        EXPECT_EQ(result, param.expected_outputs[0]);
...
TEST(HyperbolicTests, RandomAtanh) {
    ...
    for (const auto &param : test_data) {
        auto result = atanh(param.inputs[0]);
        EXPECT_EQ(result, param.expected_outputs[0]);
...
TEST(SpecialTests, RandomErf) {
    ...
    for (const auto &param : test_data) {
        auto result = erf(param.inputs[0]);
        EXPECT_EQ(result, param.expected_outputs[0]);
...
TEST(SpecialTests, RandomErfc) {
    ...
    for (const auto &param : test_data) {
        auto result = erfc(param.inputs[0]);
        EXPECT_EQ(result, param.expected_outputs[0]);
...
TEST(SpecialTests, RandomTgamma) {
    ...
    for (const auto &param : test_data) {
        auto result = tgamma(param.inputs[0]);
        EXPECT_EQ(result, param.expected_outputs[0]);
...
TEST(SpecialTests, RandomLgamma) {
    ...
    for (const auto &param : test_data) {
        auto result = lgamma(param.inputs[0]);
        EXPECT_EQ(result, param.expected_outputs[0]);
...
TEST(SpecialTests, DISABLED_RandomLerp) {
    ...
    for (const auto &param : test_data) {
        auto result = lerp(param.inputs[0], param.inputs[1], param.inputs[2]);
